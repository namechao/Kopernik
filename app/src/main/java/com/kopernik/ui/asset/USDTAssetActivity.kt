...
class USDTAssetActivity : NewBaseActivity<UYTAssetViewModel, ViewDataBinding>() {
    ...
    override fun layoutId()=R.layout.activity_usdt_asset
...
//    }
    ...
    private  fun  getCurrentAsset(){
        viewModel.run {
            getAssetConfig().observe(this@USDTAssetActivity, Observer {
                if (it.status==200){
                    allConfigEntity=it.data
    ...
    private fun getListData() {
        viewModel.run {
            if (machinngType==0) {
                ...
                rechargeCashRecord(map).observe(this@USDTAssetActivity, Observer {
                ...
                transferRecord(map).observe(this@USDTAssetActivity, Observer {
