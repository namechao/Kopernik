...
public class FingerprintHelper extends FingerprintManager.AuthenticationCallback {
    ...
    public int checkFingerprintAvailable(Context ctx) {
        if (manager == null) {
            Toast.makeText(ctx, "该设备尚不支持", Toast.LENGTH_SHORT).show();
            return -1;
        }
    ...
    public void onAuthenticationSucceeded(FingerprintManager.AuthenticationResult result) {
        ...
        if (purpose == KeyProperties.PURPOSE_DECRYPT) {
            ...
            try {
                ...
                callback.onAuthenticationSucceeded(purpose,new String(decrypted));
            ...
            try {
                ...
                if (mLocalSharedPreference.storeData(mLocalSharedPreference.dataKeyName, se) &&
                    ...
                    callback.onAuthenticationSucceeded(purpose,se);
                } else {
                    callback.onAuthenticationFail();
                }
            } catch (BadPaddingException | IllegalBlockSizeException e) {
    ...
    public interface SimpleAuthenticationCallback {
        void onAuthenticationSucceeded(int purpose, String value);
