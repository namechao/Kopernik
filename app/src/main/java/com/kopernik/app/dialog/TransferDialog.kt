...
class TransferDialog : DialogFragment(),
    ...
    private var wdbean: TransferDialogBean? = null

    @RequiresApi(api = Build.VERSION_CODES.M)
    override fun onCreateDialog(savedInstanceState: Bundle?): Dialog {
        wdbean = arguments?.getParcelable("transfer")
        val dialog =
    ...
    companion object {
        fun newInstance(type: Int, bean: Any?): TransferDialog {
            val fragment = TransferDialog()
            val bundle = Bundle()
            bundle.putInt("type", type)
            bundle.putParcelable("transfer", bean as Parcelable?)
